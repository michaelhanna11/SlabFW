...
def get_components_description(stage, G_f, G_c, Q_w, Q_m):
    ...
    if stage == "1":
        return [
            "1.35 × G_f (No γ_d)",
            "1.3 × (1.2 × G_f + 1.5 × Q_w + 1.5 × Q_m)"
        ]
    elif stage == "2":
        return [
            "1.3 × (1.35 × G_f + 1.35 × G_c)",
            "1.3 × (1.2 × G_f + 1.2 × G_c + 1.5 × Q_w + 1.5 × Q_m)"
        ]
    elif stage == "3":
        return [
            "1.3 × (1.35 × G_f + 1.35 × G_c)",
...
def main():
    ...
    **Design load = {max_load:.2f} kPa** (from Stage {critical_stage})
    """)
    st.caption("""
    Note: Calculated per AS 3610.2 strength limit state combinations with:
    - γ_d = 1.3 unanticipated load factor applied to all combinations except first dead load case
    - 1.35 factor for permanent actions
    - 1.5 factor for imposed actions
    """)
